...
function QR_By_Givens!(Q, R, A; Align = false)
    R .= A
    Q .= 0
    for p in axes(Q, 1), q in axes(Q, 2)
        Q[p, q, p] = 1
    ...
    for i = 1:(size(A, 3)-1)
        ...
        for k in axes(A, 2)
            ...
            c, s = Givens_Rotation(R[i, k, i], R[j, k, i])
            if (k > 1) && Align
                R_i_k_i = R[i, k, i] * c - R[j, k, i] * s
            ...
            #
            for p in axes(R, 3)
                tmp_i = R[i, k, p] * c - R[j, k, p] * s
                tmp_j = R[i, k, p] * s + R[j, k, p] * c
                R[i, k, p] = tmp_i
                R[j, k, p] = tmp_j
            end
            R[j, k, i] = 0
            for p in axes(Q, 1)
                tmp_i = Q[p, k, i] * c - Q[p, k, j] * s
                tmp_j = Q[p, k, i] * s + Q[p, k, j] * c
                Q[p, k, i] = tmp_i
                Q[p, k, j] = tmp_j
            end
        end
...
function QR_Shift_Iteration(AA, SH; maxiter = 12000, Align = false)
    ...
    for it = 1:maxiter
        ...
        #         display(QQ_A[:,1,:])
        #         display(SH)
        if (Max_Error < 16 * eps(Max_Value)) &&
           (Sub_Diagonals >= size(BB, 2) * div(size(BB, 1) - 1, 2))
...
function Schur_Shift(BB, SH; Align = false)
    QH = zero(BB)
    HH = zero(BB)
    ...
    #     end
    #
    CC, QQ, QQ_SH = QR_Shift_Iteration(HH, SH; Align = Align)
...
function Schur_To_Diagonal(CC, SH)
    ...
    Bundles = []
    let Index = 1
        for k = 1:size(CC_Avg, 1)-1
            if CC_Avg[k+1, k] < 32 * eps(Max_Value)
...
function Bundle_Decomposition(BB, SH; Align = true)
    CC, QQ_A, QQ_A_SH, HH = Schur_Shift(BB, SH; Align = Align)
