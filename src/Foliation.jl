...
function Foliation(
    ...
    Encoder = QPEncoder(Latent_Dimension, State_Dimension, Orthogonal_Indices, Skew_Dimension, Encoder_Order,
        node_ratio=node_ratio, leaf_ratio=leaf_ratio, max_rank=max_rank,
        Linear_Type=Linear_Type, Nonlinear_Type=Nonlinear_Type)
    return Foliation{State_Dimension,Latent_Dimension,Skew_Dimension,Model_Order,Encoder_Order}(ProductManifold(
        Model, Encoder))
end

function Make_Similar(M::Foliation{State_Dimension,Latent_Dimension,Skew_Dimension,Model_Order,Encoder_Order},
    X,
...
function Slice(M::Foliation{State_Dimension,Latent_Dimension,Skew_Dimension,Model_Order,Encoder_Order},
    X,
    Encoded_Slice,
) where {State_Dimension,Latent_Dimension,Skew_Dimension,Model_Order,Encoder_Order}
...
function Make_Cache(MF::Foliation{State_Dimension,Latent_Dimension,Skew_Dimension,Model_Order,Encoder_Order},
    XF, Index_List, Data, Encoded_Phase, Scaling;
    Model=true, Shift=false, Iterations=32, Index=0, Big_Jac=true,
    Model_IC=false,
    Time_Step = 1.0,
) where {State_Dimension,Latent_Dimension,Skew_Dimension,Model_Order,Encoder_Order}
    ...
    Model_Cache = Make_Cache(
        Model_Part(MF), Model_Point(XF), Index_List, Latent_Data, Model_Phase, Scaling, Big_Jac=Big_Jac)
    ...
    if Model_Order > 0
        if Model
            ...
            Optimise!(MF, XF, Index_List, Data, Encoded_Phase, Scaling, (1,);
                Cache=Cache, Iterations=Iterations, Time_Step=Time_Step, Index=Index, Step=0)
            Pointwise_Gradient!(Cache.Model_Gradient, Model_Part(MF), Model_Point(XF), Index_List,
                Latent_Data, Model_Phase, Scaling, Cache=Model_Cache)
        elseif Model_IC
            println("Optimising Initial Conditions!")
            Optimise_IC_Full!(Model_Part(MF), Model_Point(XF), Index_List, Latent_Data,
...
function Pointwise_Error!(Error, MF::Foliation, XF, Index_List, Data, Encoded_Phase, Scaling;
    Cache::Foliation_Cache=Make_Cache(MF, XF, Index_List, Data, Encoded_Phase, Scaling))
    Model_Phase = Get_Model_Phase(MF, Encoded_Phase)
    return Pointwise_Error!(Error, Model_Part(MF), Model_Point(XF), Index_List,
        Cache.Latent_Data, Model_Phase, Scaling, Cache=Cache.Model_Cache)
end

function Loss(MF::Foliation, XF, Index_List, Data, Encoded_Phase, Scaling;
    Cache::Foliation_Cache=Make_Cache(MF, XF, Index_List, Data, Encoded_Phase, Scaling))
    Model_Phase = Get_Model_Phase(MF, Encoded_Phase)
    return Loss(
...
function Gradient!(Gradient, MF::Foliation, XF, Index_List, Data, Encoded_Phase, Scaling, Component;
    Cache::Foliation_Cache=Make_Cache(MF, XF, Index_List, Data, Encoded_Phase, Scaling))
    Gradient_Raw = Get_Component(Cache.Gradient_Raw, Component)
    L0_DF_parts!(Gradient_Raw, Encoder_Part(MF), Encoder_Point(XF), Data, Encoded_Phase,
...
function Gradient_All!(Gradient, MF::Foliation, XF, Index_List, Data, Encoded_Phase, Scaling;
    Cache::Foliation_Cache=Make_Cache(MF, XF, Index_List, Data, Encoded_Phase, Scaling))
    ...
    while true
        ...
        Gradient!(Get_Component(Gradient, Full_Component), MF, XF, Index_List,
...
function Hessian_Raw!(Hessian, Delta, MF::Foliation, XF, Index_List, Data, Encoded_Phase, Scaling, Component;
    Cache::Foliation_Cache=Make_Cache(MF, XF, Index_List, Data, Encoded_Phase, Scaling))
...
mutable struct QPDebugTRO <: DebugAction
    ...
    function QPDebugTRO(Time_Step, radius, Component, print::Function=print)
        new(print, time(), Time_Step, [radius], Component)
    end
end
...
function Loss_Quadratic!(MF::Foliation, XF, p, p_Diff, Loss_Input, Gradient, Hessian,
...
function Gradient_Quadratic!(Gradient_p, MF::Foliation, XF, p, p_Diff, Gradient, Hessian,
    Index_List, Data, Encoded_Phase, Scaling, Component; Cache)
    #     p_Diff .= p
    #     p_Diff .-= Get_Component(XF, Component)
    #     Hessian_Raw!(Hessian, p_Diff, MF, XF, Index_List, Data, Encoded_Phase, Scaling, Component, Cache = Cache)
...
function Hessian_Quadratic!(
    ...
    riemannian_Hessian!(Get_Component(MF, Component), Hessian_p, Get_Component(XF, Component),
        Get_Component(Cache.Gradient_Raw, Component), Hessian, Delta)
    return Hessian_p
end

function Optimise!(MF::Foliation{State_Dimension,Latent_Dimension,Skew_Dimension,Model_Order,Encoder_Order}, XF,
    ...
    if Component == (1,)
        ...
        if Model_Order > 0
            ...
            println("    Model=$(Index) | -> " *
                    @sprintf("Frequency = %.5e[Hz], %.5e[rad/s] ", abs(angle(ev)) / Time_Step / (2 * pi),
        ...
        Gradient!(Get_Component(Cache.Gradient_Projected, Component), MF, XF,
        ...
        Point_Update = trust_regions!(
            ...
            Manifold,
            (M, p) -> begin
                return Loss_Quadratic!(MF::Foliation, XF, p, p_Diff, Loss_Int, Gradient_Raw_Int, Hessian_Raw_Int,
                    Index_List, Data, Encoded_Phase, Scaling, Component; Cache=Cache)
            end,
            (M, Gradient_p, p) -> begin
                Hessian_Calls = 0
                return Gradient_Quadratic!(Gradient_p, MF, XF, p, p_Diff, Gradient_Raw_Int, Hessian_Raw_Int,
                    Index_List, Data, Encoded_Phase, Scaling, Component; Cache=Cache)
            end,
            (M, Hessian_p, p, Delta) -> begin # Y -> Hessian, X -> Delta
                ...
                return Hessian_Quadratic!(Hessian_p, MF, XF, Delta, Hessian_Raw_Int, Index_List,
        ...
        if Loss_Int < Loss_Int_Update
            Point .= Point_Save
            Update_Cache!(Cache, MF, XF, Index_List, Data, Encoded_Phase, Scaling, Component)
            println("Index=", Index, " REJECT ", Component, " Margin = ",
                @sprintf("%.5e", Loss_Int - Loss_Int_Update), " UPD = ", @sprintf("%.5e", Loss_Int_Update))
        else
            println("Index=", Index, " ACCEPT ", Component, " Gain = ",
                @sprintf("%.5e", Loss_Int - Loss_Int_Update), " UPD = ", @sprintf("%.5e", Loss_Int_Update))
        end
...
function Optimise_Next!(MF::Foliation, XF, Index_List, Data, Encoded_Phase, Scaling, Step, Component_Pick;
    Cache::Foliation_Cache=Make_Cache(MF, XF, Index_List, Data, Encoded_Phase, Scaling),
    ...
    Radius = Optimise!(MF, XF, Index_List, Data, Encoded_Phase, Scaling, Component_Pick[1];
        Cache=Cache,
        Radius=Radius,
