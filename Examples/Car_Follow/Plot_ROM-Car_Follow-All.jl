...
Modes = 1
Coordinates = 2
Coordinates_Indices = [1;6;2;7;3;8;4;9;5]
fig_Auto = Figure(size = (Modes * 625, (Coordinates+1) * 250), fontsize = 32)
fig_Forced = Figure(size = (Modes * 625, (Coordinates+1) * 250), fontsize = 32)

axes_Auto_All = [Axis(fig_Auto[j,k]) for j in 1:1+Coordinates, k in 1:Modes]
axes_Forced_All = [Axis(fig_Forced[j,k]) for j in 1:1+Coordinates, k in 1:Modes]
Segment_Start = 1
Segment_End = 3
...
for k in 1:Coordinates
    axes_Auto_All[1+k,1].ylabel = L"x_{%$(Coordinates_Indices[k])}"
    axes_Forced_All[1+k,1].ylabel = L"x_{%$(Coordinates_Indices[k])}"
end
...
let Label_Id= 1
    for Index in 1:Modes
        ...
        Label_Id += 1
        for k in 1:Coordinates
            lines!(axes_Auto_All[1+k, Index], X_Axis, curves[:Data_Raw][Coordinates_Indices[k], 1+curves[:Index_List][Segment_Start]:curves[:Index_List][Segment_End]], color=Makie.wong_colors()[1])
            lines!(axes_Auto_All[1+k, Index], X_Axis, curves[:Data_Reconstructed][Coordinates_Indices[k], 1+curves[:Index_List][Segment_Start]:curves[:Index_List][Segment_End]], color=Makie.wong_colors()[2])
        ...
        if Index == 1
            ...
            xlims!(axFreq, 0.4965, 0.498)
            xlims!(axDamp, 0.0325, 0.035)
        ...
        Label_Id += 1
        for k in 1:Coordinates
            lines!(axes_Forced_All[1+k, Index], X_Axis, curves[:Data_Raw][Coordinates_Indices[k], 1+curves[:Index_List][Segment_Start]:curves[:Index_List][Segment_End]], color=Makie.wong_colors()[1])
            lines!(axes_Forced_All[1+k, Index], X_Axis, curves[:Data_Reconstructed][Coordinates_Indices[k], 1+curves[:Index_List][Segment_Start]:curves[:Index_List][Segment_End]], color=Makie.wong_colors()[2])
