...
for k in 1:Modes
    axes_Auto_All[1+k,1].ylabel = "Mode $(k) [mm]"
    axes_Forced_All[1+k,1].ylabel = "Mode $(k) [mm]"
end
...
let Label_Id= 1
    for Index in 1:Modes
        ...
        curves = JLSO.load("../Foil-Auto/CURVES-$(Name)-$(VER)-I$(Index).bson")
    ...
    #     MAP_Backbone = curves[:MAP_Backbone]
        ...
        fig = Create_Plot(Amplitude = "Amplitude [mm]")
    ...
    #     Plot_Backbone_Curves!(fig, MAP_Backbone, Data_Max; Label = "Map aut", Color = Makie.wong_colors()[2])
        Plot_Backbone_Curves!(fig, DATA_Backbone, Data_Max; Label = "Data aut", Color = Makie.wong_colors()[1], Amplitude_Scale = mm_per_px)
        Plot_Error_Curves!(fig, DATA_Error_Curves, Data_Max; Label = "Data aut", Color = Makie.wong_colors()[1], Amplitude_Scale = mm_per_px)
        Plot_Error_Curves!(fig, TEST_Error_Curves, Data_Max; Label = "Data aut", Color = Makie.wong_colors()[1],
                        Dense_Style = :dash,  Max_Style = :dashdot, Mean_Style = :dash, Amplitude_Scale = mm_per_px)
        ...
        if Index == 2
            ...
            ylims!(axFreq, 0, 6 * mm_per_px)
            ylims!(axDamp, 0, 6 * mm_per_px)
            ylims!(axDense, 0, 6 * mm_per_px)
            ylims!(axErr, 0, 6 * mm_per_px)
        end
        # Annotate_Plot!(fig)
        ...
        Label_Id += 1
        for k in 1:Modes
            lines!(axes_Auto_All[1+k, Index], X_Axis, mm_per_px * curves[:Data_Raw][k, 1+curves[:Index_List][Segment_Start]:curves[:Index_List][Segment_End]], color=Makie.wong_colors()[1])
            lines!(axes_Auto_All[1+k, Index], X_Axis, mm_per_px * curves[:Data_Reconstructed][k, 1+curves[:Index_List][Segment_Start]:curves[:Index_List][Segment_End]], color=Makie.wong_colors()[2])
    ...
    #     Plot_Backbone_Curves!(fig, MAP_Backbone, Data_Max; Label = "Map forced", Color = Makie.wong_colors()[5])
        Plot_Backbone_Curves!(fig, DATA_Backbone, Data_Max; Label = "Data forced", Color = Makie.wong_colors()[4], Amplitude_Scale = mm_per_px)
        Plot_Error_Curves!(fig, DATA_Error_Curves, Data_Max; Label = "Data forced", Color = Makie.wong_colors()[4], Amplitude_Scale = mm_per_px)
        ...
        Label_Id += 1
        for k in 1:Modes
            lines!(axes_Forced_All[1+k, Index], X_Axis, mm_per_px * curves[:Data_Raw][k, 1+curves[:Index_List][Segment_Start]:curves[:Index_List][Segment_End]], color=Makie.wong_colors()[1])
            lines!(axes_Forced_All[1+k, Index], X_Axis, mm_per_px * curves[:Data_Reconstructed][k, 1+curves[:Index_List][Segment_Start]:curves[:Index_List][Segment_End]], color=Makie.wong_colors()[2])
...
CairoMakie.activate!(type = "svg")
save("FIGURE-MODES-Autonomous.svg", fig_Auto)
save("FIGURE-MODES-Forced.svg", fig_Forced)
