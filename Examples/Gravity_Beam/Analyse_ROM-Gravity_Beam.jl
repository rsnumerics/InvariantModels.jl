...
Index = 1
Data_Radius_List = (1.0, 0.82,)
Implicit_Radius_List = (1.2, 1.0,)
...
PPM, PPX = Find_DATA_Manifold(
    ...
    Transformation = 8 * Data_Decoder[1:1, :, :],
    abstol = 1e-9,
    reltol = 1e-9,
    maxiters = 36,
...
#     Output_Transformation = Data_Encoder,
    Output_Inverse_Transformation = Data_Decoder[1:1,:,:],
...
MTF_Cache, DATA_Backbone, DATA_Error_Curves, Data_Max = Data_Result(
    ...
    Data_Decomp,
    Encoded_Phase,
    Transformation = 200 * Data_Decoder[1:1, :, :] ./ reshape(Data_Scale, 1, 1, :),
    Time_Step = Time_Step,
    Hz = false,
